...
public class SwipeDetector : MonoBehaviour
{
    ...
    void FindPiece()
    {
        ...
        if (InventoryLogic.instance.transform.parent.GetComponent<Collider2D>().OverlapPoint(fingerDownPosition)) onInventory = true;
        else if (onInventory) onInventory = false;
        ...
        selectedPiece.GetComponent<JigsawPieceLogic>().updateSortingOrder = true;
        offset = (Vector2)selectedPiece.transform.position - fingerDownPosition;
    }
    ...
    private void Update()
    {
        foreach (Touch touch in Input.touches)
        {
            ...
            if (selectedPiece)
                if (!onInventory)
                    selectedPiece.transform.position = fingerDownPosition + offset;
            ...
            if (onInventory) InventoryLogic.instance.transform.position = new Vector2(InventoryLogic.instance.transform.position.x, InventoryLogic.instance.transform.position.y + (fingerDownPosition.y - fingerUpPosition.y));
        }
    }
